...
public class Main {
    ...
    //метод для нахождения победной строки (набора)
    private static boolean checkSet(int x, int y, char sym){
        int flag1 = 0, flag2 = 0, flag3 = 0, flag4 = 0;

        for(int step = 0; step < WinSet; step++) {
            // проверяем победную серию справа
            if (x + step < SIZE_X) {
                if (field[y][x + step] == sym) {
                    flag1++;
                }
            }
            // проверяем победную серию снизу
            if (y + step < SIZE_Y) {
                if (field[y + step][x] == sym) {
                    flag2++;
                }
            }
            // проверяем победную серию по правой диагонали вниз
            if (x + step < SIZE_X && y + step < SIZE_Y) {
                if (field[y + step][x + step] == sym) {
                    flag3++;
                }
            }
            // проверяем победную серию по левой диагонали вниз
            if (x - step >= 0 && y + step < SIZE_Y) {
                if (field[y + step][x - step] == sym) {
                    flag4++;
                }
            }
            if (flag1 == WinSet || flag2 == WinSet || flag3 == WinSet || flag4 == WinSet) {
                return true;
            }
        }
        return false;
